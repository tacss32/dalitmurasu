...
const createPost = async (req, res) => {
  try {
    ...
    if (isPinned) {
      const pinnedCount = await UniversalPost.countDocuments({
        isPinned: true,
      });
      if (pinnedCount >= 3) {
        return res
          .status(400)
          .json({ error: "Maximum of 3 pinned posts allowed" });
      }
    }
...
// Delete Universal Post + Unpin if pinned
const deletePost = async (req, res) => {
  try {
    const { id } = req.params;

    // 1️⃣ Remove from pinned posts (safe even if not pinned)
    await PinnedPost.deleteMany({
      postId: id,
      source: "UniversalPost",
    });

    // 2️⃣ Delete the universal post
    const deletedPost = await UniversalPost.findByIdAndDelete(id);
    ...
    console.error("Error deleting universal post:", err);
    res.status(500).json({
...
const updatePost = async (req, res) => {
  try {
    const {
      ...
      date,
      existingImages,
    } = req.body; // ✅ Destructure existingImages
...
const getPinnedPosts = async (req, res) => {
  try {
    const pinnedPosts = await UniversalPost.find({ isPinned: true }).sort({
      createdAt: -1,
    });
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch pinned posts", details: err.message });
  }
};
...
const unpinPost = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to unpin post", details: err.message });
  }
};

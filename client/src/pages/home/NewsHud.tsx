...
export default function NewsHud() {
  ...
  const fetchCategories = async () => {
    try {
      const categoriesRes = await axios.get<Category[]>(
        `${API_BASE_URL}api/categories?available=true`
      );
  ...
  // This list includes all pinned posts (up to 3) and fills the rest with universal posts, up to a total of 12.
  const combinedPosts = [
    ...pinnedPosts,
    ...universalPosts.slice(0, 12 - pinnedPosts.length),
  ];

  // The total number of posts to display in the main grid, based on the combined list.
  const postsToShow = combinedPosts.slice(0, 12);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {homeBooks.slice(0, 4).map((book) => (
              ...
              >
                ...
                >
                  {addingToCartStates.has(book._id)
                    ? "Adding..."
                    : "Add to Cart"}
                </button>
              </div>

...
const AddPhotoForm: React.FC = () => {
  ...
  const [cartoonsImages, setCartoonsImages] = useState<CartoonsImage[]>([]);
  const [cartoonsLoading, setCartoonsLoading] = useState<boolean>(true); // Existing cartoons loader
  const [cartoonsError, setCartoonsError] = useState<string>("");
  ...
  // --- Fetch Cartoons Images ---
  const fetchCartoonsImages = useCallback(async () => {
    setIsLoadingFullPage(true);
    setCartoonsLoading(true);
    setCartoonsError("");
    try {
      const res = await fetch(`${API_BASE_URL}api/cartoons`);
      if (!res.ok) {
        throw new Error(`HTTP error! status: ${res.status}`);
      }
      const data: CartoonsImage[] = await res.json();
      setCartoonsImages(data);
    } catch (err) {
      console.error("Error fetching cartoons images:", err);
      setCartoonsError("Failed to load cartoons images.");
    } finally {
      setCartoonsLoading(false);
  ...
  const handleEditClick = (image: CartoonsImage) => {
    setSuccessMessage("");
    setErrorMessage("");
  ...
  const handleFormSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const url = isUpdating
        ? `${API_BASE_URL}api/cartoons/${editingId}`
        : `${API_BASE_URL}api/cartoons`;
  ...
  const handleDeleteClick = async (imageId: string) => {
    ...
    if (confirmDelete) {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}api/cartoons/${imageId}`, {
          method: "DELETE",
        });
        const data = await response.json();
        if (response.ok) {
    ...
    >
      ...
      <div className="mt-8 pt-6 border-t">
        <h3 className="text-2xl font-bold text-gray-800 text-center mb-6">
          Manage Cartoons Images
        </h3>
        {cartoonsLoading && <p className="text-center">Loading images...</p>}
        {cartoonsError && (
          <p className="text-center text-red-600">{cartoonsError}</p>
        )}
        {cartoonsImages.length === 0 && !cartoonsLoading && !cartoonsError && (
          <p className="text-center text-gray-500">No images uploaded yet.</p>
        )}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {cartoonsImages.map((img) => (
            <div
              key={img._id}
